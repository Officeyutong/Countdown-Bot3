...
def render_latex(formula: str) -> bytes:
    ...
    sympy.preview(formula, viewer="BytesIO", euler=False,
                  outputbuffer=buf, packages=tuple(["amssymb", "color", "amsthm", "multirow", "enumerate", "amstext"]))
    return buf.getvalue()
...
output = plot(-3.1415926535, 3.1415926535, ["sin(x)"])
# print(output)
with open("output.txt", "w") as f:
    import base64
    output["image"] = base64.encodebytes(
        output["image"]).decode().replace("\n", "")
    import json
